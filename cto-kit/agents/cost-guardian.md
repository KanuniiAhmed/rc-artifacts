...
- **A budget alarm exists** (e.g. AWS Budgets) with a real monthly ceiling and a notification target. If the project states a cap (e.g. "$50/month"), verify the alarm matches it.
- **Nothing runs that can't stop itself.** Every compute resource (EC2/GPU/containers) has auto-shutdown or is spot/ephemeral with a start/stop mechanism. No always-on instance without an explicit, justified reason.
- **Storage is bounded.** Object storage has lifecycle rules; no unbounded growth. Ephemeral data (downloaded media, temp artifacts) is deleted after use with a lifecycle rule as backstop.
...
- Spot/preemptible over on-demand for batch/GPU work; smallest instance that meets the need.
