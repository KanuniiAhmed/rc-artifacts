...
- **Input trust:** every external input (user forms, paid APIs, scraped content, uploaded files, URLs handed to fetchers/downloaders) is untrusted until validated. Check for injection (SQL/NoSQL/command/prompt), SSRF in any server-side fetcher, unsafe deserialization, and path traversal. Enforce strict allowlists where a tool takes arbitrary URLs (e.g. a URL-fetching CLI, an HTTP client, image fetchers) — that is an injection surface.
...
- **Abuse & rate limiting:** rate limits on auth and public endpoints; automated-fetcher rate limits and per-target robots.txt / ToS compliance.
...
- **Project NATURE / domain disclosure (subtle — hunt for it aggressively).** Even with codenames removed, reusable/public instructions, prompts, examples, and docs must be **domain-neutral** — they must not let a reader infer *what the owner's private projects are or do*. Flag and genericize any tell: specific **budget/cost figures**, **tech-stack specifics** that imply a niche workload (a named single-purpose CLI tool, a specific accelerator type, a specific ingestion/download method, a named framework), **domain vocabulary** (industry/product jargon — entity nouns specific to one field), a **named human language or region**, and concrete data-model nouns lifted from a real product. Replace with neutral placeholders ("a documented budget cap", "a compute-heavy workload", "an RTL language", "the domain's primary entities", "a downstream consumer"). Also make sure the checklist's *own examples* don't echo a real instance elsewhere in the repo — a matching "hypothetical" confirms it wasn't hypothetical. The test for every example sentence: *could a reader reverse-engineer what my other projects do from this?* If yes, generalize it. Keep the engineering principle; drop the fingerprint.
